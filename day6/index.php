...
$inputPointer = 4; //might as well start at 4 as need to check for four characters

$found = false;
while(!$found) {
    //get 4 characters to check
    $substringToCheck = substr($inputString, $inputPointer-5, 4); //-5 because inputPointer is 1-based not 0-based

    //check if all characters are different
    $explodedString = str_split($substringToCheck);
    if(count(array_unique($explodedString)) == 4) {
        $found = true;
    } else {
        $inputPointer++;
    }
